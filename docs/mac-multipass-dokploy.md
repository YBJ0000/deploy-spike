...
| 2026-03-11 | 第五步   | 浏览器访问 `http://<VM IP>:3000` 提示 `ERR_CONNECTION_REFUSED`，但 `multipass list` 显示 VM 仍为 `Running` | 一般说明「VM 在，但 Dokploy 自身（Docker 服务或 Dokploy 容器）没在监听 3000 端口」。**处理顺序**：1）确认 `multipass list` 中的 IP 是否变化（例如从 `192.168.64.3` 变成 `192.168.64.4`）并改用最新 IP 访问；2）`multipass shell dokploy-vm` 进入 VM，执行 `sudo docker ps` 确认是否有 Dokploy 相关容器在跑（名称通常包含 `dokploy`）；3）若没有，尝试 `dokploy start` 或在 `/etc/dokploy` 下执行 `sudo docker compose up -d`（视安装脚本的实际路径而定）；4）如仍不行，重启 VM：`multipass restart dokploy-vm`，几分钟后再访问 `http://<VM IP>:3000`。 |
| 2026-03-11 | 第五步   | `multipass stop dokploy-vm && multipass start dokploy-vm` 后可正常访问 Dokploy；重新 Deploy medical-server 的 Compose 后，容器短暂 running，约几十秒–1 分钟后整台 VM 再次失联（`multipass shell` 与 3000 端口均超时） | 高度怀疑为 **VM 本身资源不足**（内存/磁盘）导致在 Mongo + RabbitMQ + Redis + Raidar 同时运行时系统卡死。**处理建议**：1）在宿主机清理本地 Docker 镜像/卷，确保有足够磁盘空间；2）若该 VM 多次在同一负载下挂死，优先考虑删除重建一台资源更高的 VM（例如 `multipass launch 22.04 --name dokploy-vm --memory 4G --disk 40G --cpus 2`），重新安装 Dokploy 并按文档重新部署。 |
